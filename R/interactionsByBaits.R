...
interactionsByBaits <- function(interactions, baits, sep=",")
{
  ...
  else
  {
    ...
    baits_final <- dplyr::as_tibble(rbind(baits1,baits2)) %>%
      dplyr::group_by(bait_1) %>% dplyr::summarise(ID_1=unique(ID_1),
    ...
    #   dplyr::group_by(ID_1) %>%
    #   dplyr::summarise(interactingDistance=paste(distance,collapse = ",")) %>%
    #   dplyr::rename("ID" = "ID_1")
    #
    # baits_final <- dplyr::as_tibble(merge(baits_final,baits_final2,by="ID"))
